...
#include "sitl_gazebo/gazebo_wind_plugin.h"
#include "sitl_gazebo/common.h"

namespace gazebo {

typedef const boost::shared_ptr<const ignition::msgs::Boolean> BooleanPtr;
...
void GazeboWindPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  wind_toggle_sub = node_handle_->Subscribe("~/wind_toggle", &GazeboWindPlugin::WindToggleCallback, this);
...
{
  this->wind_status = msg->data();
  gzmsg << "Wind toggle" << std::endl;
}
